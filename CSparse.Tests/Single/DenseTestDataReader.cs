...
namespace CSparse.Tests.Single
{
    ...
    {
        public static DenseTestData<Real, Real> Read(Stream stream)
        {
        ...
        private static DenseTestData<Real, Real> ReadSize(string line)
        {
            var size = line.Split();

            return new DenseTestData<Real, Real>()
            {
        ...
        private static DenseMatrix ReadMatrix(string line, int m, int n)
        {
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    result.At(i, j, Real.Parse(row[j], CultureInfo.InvariantCulture));
                }
            }
        ...
        private static Real[] ReadVector(string line, int size)
        {
            ...
            var result = new Real[size];

            for (int i = 0; i < size; i++)
            {
                result[i] = Real.Parse(values[i], CultureInfo.InvariantCulture);
            }
